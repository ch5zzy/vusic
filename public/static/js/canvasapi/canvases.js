...
import getCanvases from './_canvasApi.js';
...
export async function getTrackCanvases(tracks) {
  ...
  let canvasResponse = await getCanvases(uniqueTracks, await canvasToken);
...
export async function getRecentCanvases(req, res) {
  ...
  let canvases = [];  // response array
  let canvasToken = getCanvasToken();
  let personalToken = req.query.access_token;
...
function getPersonalToken(refreshToken) {
  ...
  return axios.post(PERSONAL_TOKEN_URL, stringify(personalTokenForm), personalTokenOpts)
    .then(response => {
      if (response.statusText !== 'OK') {
