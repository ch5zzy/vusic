...
let trackCanvas = document.querySelector("#track-canvas");
...
spotifyApi.updateFunc = () => {
  if (spotifyApi.trackTitle != "") {
    ...
    circleText += " · " + spotifyApi.trackArtistsNames.toLowerCase() +
      (spotifyApi.trackAlbum != "" ? (" · " + spotifyApi.trackAlbum.toLowerCase()) : "");
    trackImg.src = spotifyApi.trackImage;

    let trackColor = spotifyApi.trackColor;

    if (spotifyApi.trackCanvas) {
      trackCanvas.src = spotifyApi.trackCanvas;
      videoCanvasCtx.drawImage(trackCanvas, 0, 0, trackCanvas.offsetWidth / 2, trackCanvas.offsetHeight / 2);
      const videoColor = videoCanvasCtx.getImageData(0, 0, 1, 1).data;
      trackColor = new Color(videoColor[0], videoColor[1], videoColor[2]);
